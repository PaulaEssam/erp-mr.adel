...
if (isset($_POST['delete'])) {
    ...
    $katkot_id = $_POST['katkot_id'];
    $katkot = $conn->prepare("DELETE FROM `kataket` WHERE id= ?");
    $katkot->execute([$katkot_id]);
}
...
<section class="comments">
    <?php
        $select_baqy = $conn->prepare("SELECT * FROM `kataket` WHERE name = ? ORDER BY id DESC ");
        ...
        <div class="box">
            <table id="studentTable" class="studentsTable">
                ...
                <tbody>
                    <?php
                    $i=1;
                        $select_katkot = $conn->prepare("SELECT * FROM `kataket` WHERE name=?");
