...
include 'components/connect.php';
    ...
    if (isset($_POST['submit'])) {    
        ...
        // }
        $rest = 0;
        $select_katkot = $conn->prepare("SELECT * FROM `kataket` WHERE name=? ORDER BY id DESC");
        $select_katkot->execute([$u_name]);
        $fetch_katkot = $select_katkot->fetch(PDO::FETCH_ASSOC);

        if ($select_katkot->rowCount() > 0) {
            
            $total_pay = $pay + $fetch_katkot['total_pay'];
            if (!empty($price)) {
                $total_price = $price + $fetch_katkot['total_price'];
            }
            else {
                $total_price = $fetch_katkot['total_price'];
            }
        }
            $rest = $total_price - $total_pay ;
            $add_content = $conn->prepare("INSERT INTO `kataket` ( name, num, price, total_price, pay, total_pay, rest, created_at)
            VALUES (?,?,?,?,?,?,?,?)");
...
<section class="crud-form">
    <h1 class="heading">اضافة كتاكيت</h1>
